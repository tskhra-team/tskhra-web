...
const benefitKeys = [
  { icon: RefreshCw, key: "smartMatching" },
  { icon: Users, key: "verifiedCommunity" },
  { icon: Globe, key: "diversity" },
];

const categoryKeys = [
  { key: "books", count: "12.5K", color: "from-amber-500 to-orange-500" },
  { key: "electronics", count: "8.2K", color: "from-blue-500 to-indigo-500" },
  { key: "furniture", count: "5.8K", color: "from-green-500 to-emerald-500" },
  { key: "services", count: "15.3K", color: "from-purple-500 to-pink-500" },
];
          ...
          {benefitKeys.map((benefit, index) => (
            ...
            >
              ...
              <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2">
                {t(`swapSection.features.${benefit.key}.title`)}
              </h3>
              <p className="text-sm sm:text-base text-slate-600 leading-relaxed">
                {t(`swapSection.features.${benefit.key}.description`)}
              </p>
            </div>
          ))}
        </div>
            ...
            {categoryKeys.map((category, index) => (
              ...
              >
                ...
                <h4 className="text-base sm:text-lg font-bold text-slate-900 mb-1">
                  {t(`swapSection.categoryList.${category.key}.name`)}
                </h4>
                <p className="text-xs sm:text-sm text-slate-500 font-medium">
                  {category.count} {t("swapSection.activeSwaps")}
                </p>
              </div>
            ))}
          </div>
